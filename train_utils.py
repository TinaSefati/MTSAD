...
def plot_signals(input_data, output_data, name):
    # Plot real data
...
def plotSave_signals(input_data, output_data, model_name, epoch, is_train):
    # Define the directory to save figures
    save_dir = os.path.expanduser("E:\FIGS")
    ...
    plt.figure()
    plt.plot(input_data[-1].to('cpu').detach().numpy())
    plt.title(f"Real Data - {model_name} - Epoch {epoch} - {data_type.capitalize()}")
    plt.xlabel("Time")
    plt.ylabel("Value")
    # Save the figure
    real_data_path = os.path.join(save_dir, f"RealData_{model_name}_Epoch{epoch}_{data_type}.png")
    plt.savefig(real_data_path)
    plt.close()

    # Plot reconstructed data
    plt.figure()
    plt.plot(output_data[-1].to('cpu').detach().numpy())
    plt.title(f"Reconstructed Data - {model_name} - Epoch {epoch} - {data_type.capitalize()}")
    plt.xlabel("Time")
    plt.ylabel("Value")
    # Save the figure
    reconstructed_data_path = os.path.join(save_dir, f"ReconstructedData_{model_name}_Epoch{epoch}_{data_type}.png")
    plt.savefig(reconstructed_data_path)
    plt.close()
...
def evaluate_epoch(epoch, num_epochs, model, test_loader, device, target_list):
    ...
    print('FP = ', FP)
    print('FN = ', FN)
    STATS_FILE = os.path.join('E:\FIGS', 'results-ablation.csv')
    ...
    with open(STATS_FILE, 'a', newline='') as csvfile:
        ...
        if not file_exists:
            writer.writerow([
                'Epoch','Test Loss','AUC', 'Accuracy', 'Precision',
                'Recall', 'F1', 'TP', 'TN', 'FP', 'FN', 'Best Threshold'])

        writer.writerow([
            epoch + 1,avg_test_loss, auc,accuracy, precision, recall, f1s_score, TP, TN, FP, FN,best_threshold])

    return eval_time, input_data, output_data
